...
static int ggmlop_dsp_mulmat_singlethread(remote_handle64 h, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // 1. NULL 포인터, data 등 체크
    if (!src0) { GGMLHEXAGON_LOG_DEBUG("src0 is NULL"); return -1; }
    if (!src1) { GGMLHEXAGON_LOG_DEBUG("src1 is NULL"); return -2; }
    if (!dst)  { GGMLHEXAGON_LOG_DEBUG("dst is NULL"); return -3; }

    if (!src0->data) { GGMLHEXAGON_LOG_DEBUG("src0->data is NULL"); return -4; }
    if (!src1->data) { GGMLHEXAGON_LOG_DEBUG("src1->data is NULL"); return -5; }
    if (!dst->data)  { GGMLHEXAGON_LOG_DEBUG("dst->data is NULL"); return -6; }

    // 2. 모든 shape/stride/type 값, 실제 주소 등 다 출력
    GGMLHEXAGON_LOG_DEBUG("src0: data=%p, type=%d, ne={%ld,%ld,%ld,%ld}, nb={%ld,%ld,%ld,%ld}",
        src0->data, src0->type, src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
        src0->nb[0], src0->nb[1], src0->nb[2], src0->nb[3]);
    GGMLHEXAGON_LOG_DEBUG("src1: data=%p, type=%d, ne={%ld,%ld,%ld,%ld}, nb={%ld,%ld,%ld,%ld}",
        src1->data, src1->type, src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3],
        src1->nb[0], src1->nb[1], src1->nb[2], src1->nb[3]);
    GGMLHEXAGON_LOG_DEBUG("dst:  data=%p, type=%d, ne={%ld,%ld,%ld,%ld}, nb={%ld,%ld,%ld,%ld}",
        dst->data, dst->type, dst->ne[0], dst->ne[1], dst->ne[2], dst->ne[3],
        dst->nb[0], dst->nb[1], dst->nb[2], dst->nb[3]);

    // 3. shape, stride, type, pointer 값이 이상할 때(음수, 0, 비정상값 등)
    for (int i = 0; i < 4; ++i) {
        if (src0->ne[i] < 0 || src1->ne[i] < 0 || dst->ne[i] < 0) {
            GGMLHEXAGON_LOG_DEBUG("ne[%d] < 0: src0=%ld src1=%ld dst=%ld", i, src0->ne[i], src1->ne[i], dst->ne[i]);
            return -10;
        }
        if (src0->nb[i] < 0 || src1->nb[i] < 0 || dst->nb[i] < 0) {
            GGMLHEXAGON_LOG_DEBUG("nb[%d] < 0: src0=%ld src1=%ld dst=%ld", i, src0->nb[i], src1->nb[i], dst->nb[i]);
            return -11;
        }
    }
    ...
    dst->nb[3] = dst->nb[2] * dst->ne[2];
    ggmlhexagon_dump_tensor(dst, 0);
    GGMLHEXAGON_LOG_DEBUG("check_1 %s", __func__ );
    GGML_TENSOR_BINARY_OP_LOCALS
    GGMLHEXAGON_LOG_DEBUG("check_2 %s", __func__ );
    int32_t  const vec_dot_num_rows     = 1;
    GGMLHEXAGON_LOG_DEBUG("ne0=%d, ne1=%d, ne2=%d, ne3=%d", ne0, ne1, ne2, ne3);
    GGMLHEXAGON_LOG_DEBUG("ne01=%d, ne11=%d, ne12=%d, ne13=%d", ne01, ne11, ne12, ne13);
    GGMLHEXAGON_LOG_DEBUG("nb00=%d, nb10=%d, nb0=%d, nb1=%d, nb2=%d, nb3=%d", nb00, nb10, nb0, nb1, nb2, nb3);
    ...
    GGML_ASSERT(nb1 <= nb2);
    GGML_ASSERT(nb2 <= nb3);
    GGMLHEXAGON_LOG_DEBUG("check_5 %s", __func__ );
...
int ggmlop_dsp_mulmat(remote_handle64 h, const struct dsptensor * src0, const struct dsptensor * src1, dsptensor * dst) {
    //remote_debug_printf("AAAAAAAAAAAAAAAAA");
    // volatile int dummy = 0;
    // for (volatile long i = 0; i < 1000000000; i++) {
    //     dummy += i;
    // }
    // qurt_printf("AAAAAAAAAAAAAAAAAAAAA");
    GGMLHEXAGON_LOG_INFO("AAAAAAAAAAA");
