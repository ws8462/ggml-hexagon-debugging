...
void * qnn_instance::alloc_rpcmem_internal(size_t bytes, size_t alignment) {
    ...
    // 디버깅 출력 추가
    std::cout << "[RPCMEM DEBUG] bytes: " << bytes
            << ", alignment: " << alignment
            << ", total (allocate_bytes): " << allocate_bytes << std::endl;
...
int qnn_instance::register_rpcmem(void * p_data, Qnn_Tensor_t * p_tensor) {
    ...
    Qnn_MemDescriptor_t descriptor = {
        .memShape = {QNN_VER_PTR(*p_tensor)->rank, QNN_VER_PTR(*p_tensor)->dimensions, nullptr},
        .dataType = QNN_VER_PTR(*p_tensor)->dataType,
        .memType = QNN_MEM_TYPE_ION, //QNN_MEM_TYPE_ION
        .ionInfo.fd = mem_fd};
    ...
    GGMLHEXAGON_LOG_DEBUG("name : %s : mem_handle copy %p, original %p", (QNN_VER_PTR(*p_tensor)->name), QNN_VER_PTR(*p_tensor)->memHandle, handle);
...
//     GGMLHEXAGON_LOG_DEBUG("__mem_fd %d", mem_fd);
//     Qnn_MemDescriptor_t descriptor = {
...
void * qnn_instance::get_rpcmem_from_memhandle(Qnn_MemHandle_t mem_handle) {
    ...
    for (const auto & [ptr, handle] : _qnn_mem_set) {
        GGMLHEXAGON_LOG_DEBUG("rpcmem address : %p, handle : %p", ptr, handle);
    }
    for (const auto & [ptr, handle] : _qnn_mem_set) {
...
int qnn_instance::load_backend(std::string & lib_path, const QnnSaver_Config_t ** saver_config) {
    ...
    std::cout << "[DEBUG] QNN_API_VERSION_MAJOR: " << QNN_API_VERSION_MAJOR << std::endl;
    std::cout << "[DEBUG] QNN_API_VERSION_MINOR: " << QNN_API_VERSION_MINOR << std::endl;

    for (size_t idx = 0; idx < num_providers; idx++) {
        auto major = provider_list[idx]->apiVersion.coreApiVersion.major;
        auto minor = provider_list[idx]->apiVersion.coreApiVersion.minor;
        std::cout << "[DEBUG] QNN_API_Checking provider " << idx
...
int qnn_instance::load_system() {
    ...
    std::cout << "[DEBUG] QNN_SYSTEM_API_VERSION_MAJOR: " << QNN_SYSTEM_API_VERSION_MAJOR << std::endl;
    std::cout << "[DEBUG] QNN_SYSTEM_API_VERSION_MINOR: " << QNN_SYSTEM_API_VERSION_MINOR << std::endl;


    QNN_SYSTEM_INTERFACE_VER_TYPE qnn_system_interface;
    bool found_valid_system_interface = false;
    for (size_t idx = 0; idx < num_providers; idx++) {
        auto major = provider_list[idx]->systemApiVersion.major;
        auto minor = provider_list[idx]->systemApiVersion.minor;
        std::cout << "[DEBUG] Checking provider " << idx
                << " → version: " << major << "." << minor << std::endl;
...
int qnn_instance::qnn_init(const QnnSaver_Config_t ** saver_config) {
    ...
    if (_device_id == HEXAGON_BACKEND_QNNNPU) {
        ...
        if (QNN_SUCCESS == qnnstatus) {
            ...
            for (uint32_t i = 0; i < p_info->v1.numHwDevices; i++) {
                ...
                GGMLHEXAGON_LOG_INFO("  - Arch Type   : (%zu)", htp_arch);
                GGMLHEXAGON_LOG_INFO("  - SoC Model   : %u", (unsigned int) chipinfo.socModel);
                GGMLHEXAGON_LOG_INFO("  - VTCM Size   : %zu bytes", chipinfo.vtcmSize);
    ...
    if (QNN_SUCCESS != qnnstatus && QNN_DEVICE_ERROR_UNSUPPORTED_FEATURE != qnnstatus) {
        GGMLHEXAGON_LOG_WARN("QNN device create failed with status: %d\n", QNN_GET_ERROR_CODE(qnnstatus));
...
void qnn_instance::htp_probe_rpc_meminfo() {
    ...
    size_t probe_slots[]    = {1024, 1536, 2048 - 48}; //{1024, 1536, 2048 - 48, 2048};
...
void qnn_instance::htp_enter_performance_mode() {
    ...
    if (ret != QNN_SUCCESS) {
        GGMLHEXAGON_LOG_WARN("failed to set HTP power config, ret = %d", ret);
    } else {
...
static uint8_t * ggmlqnn_create_rpc_buffer(qnn_instance * instance, const ggml_tensor * ggml_tensor, Qnn_Tensor_t * qnn_tensor, bool b_copydata) {
    ...
    uint8_t * qnn_rpcbuffer = static_cast<uint8_t *>(instance->alloc_rpcmem(ggml_nbytes(ggml_tensor), 4096)); // 4
    ...
    GGMLHEXAGON_LOG_DEBUG("1_name : %s : mem_handle %p", (QNN_VER_PTR(*qnn_tensor)->name), QNN_VER_PTR(*qnn_tensor)->memHandle);
...
void hexagon_quantize_q8(const ggml_tensor * src,
                         int8_t ** out_data,
                         float ** out_scales,
                         uint32_t * out_rows,
                         uint32_t * out_cols,
    ...
    const int64_t nrow = src->ne[1];
    const int64_t ncol = src->ne[0];
    const int block_size = 32;
    GGML_ASSERT(ncol % block_size == 0);

    const int blocks_per_row = ncol / block_size;
    const int total_blocks = nrow * blocks_per_row;

    const float * src_data = (const float *) src->data;

    *out_data = (int8_t *) malloc(total_blocks * block_size);
    *out_scales = (float *) malloc(total_blocks * sizeof(float));

    GGML_ASSERT(*out_data != nullptr);
    GGML_ASSERT(*out_scales != nullptr);

    printf("[quantize_q8] nrow = %" PRId64 ", ncol = %" PRId64 "\n", nrow, ncol);
    printf("[quantize_q8] total_blocks = %d\n", total_blocks);

    for (int64_t row = 0; row < nrow; ++row) {
        for (int b = 0; b < blocks_per_row; ++b) {
            const float * x = src_data + row * ncol + b * block_size;
            float amax = 0.0f;
            ...
            (*out_scales)[row * blocks_per_row + b] = d;

            printf("[quantize_q8] row=%" PRId64 ", block=%d, amax=%.6f, scale(d)=%.6f\n",
                   row, b, amax, d);

            for (int i = 0; i < block_size; ++i) {
                int8_t qval = (int8_t) roundf(x[i] / d);
                (*out_data)[row * blocks_per_row * block_size + b * block_size + i] = qval;

                if (i < 4) { // 앞부분 4개만 프린트
                    printf("    x[%2d] = %+9.6f → q = %+4d\n", i, x[i], qval);
                }
            }
        }
    }
    ...
    *out_rows = nrow;
    *out_cols = ncol;
    *out_data_size = total_blocks * block_size;

    printf("[quantize_q8] output shape: [%d, %d], data_size: %u bytes\n",
           *out_rows, *out_cols, *out_data_size);
}
...
static Qnn_Tensor_t * ggmlqnn_create_general_tensor(qnn_instance * instance, Qnn_GraphHandle_t graph_handle,
    ...
    //Tensor type 기반 QNN data type 분기
    if (nullptr != tensor) {
        switch (tensor->type) {
            case GGML_TYPE_Q4_0:
            case GGML_TYPE_Q4_1:
    ...
    if (tensor != nullptr) {
        ...
        GGMLHEXAGON_LOG_DEBUG("ggml_tensor name: %s", name ? name : "null");
        GGMLHEXAGON_LOG_DEBUG("ggml_tensor type: %s (%d)", type_name ? type_name : "null", type);
        GGMLHEXAGON_LOG_DEBUG("tensor size: %zu", size);
    } else {
        GGMLHEXAGON_LOG_WARN("ggml_tensor is nullptr, skipping name/type/size debug info.");
    }
    bool enable_npu_rpc = (instance->enable_qnn_rpc() && instance->get_device_id() == HEXAGON_BACKEND_QNNNPU);
    Qnn_MemHandle_t handle = nullptr;
    if (enable_npu_rpc && (qnn_tensor_type == QNN_TENSOR_TYPE_APP_WRITE || qnn_tensor_type == QNN_TENSOR_TYPE_APP_READ)) { // (qnn_tensor_type == QNN_TENSOR_TYPE_APP_WRITE || qnn_tensor_type == QNN_TENSOR_TYPE_APP_READ)
        ...
        //     return nullptr;
        // }
        //GGMLHEXAGON_LOG_DEBUG("2_name : %s : mem_handle %p", (QNN_VER_PTR(*p_qnn_tensor)->name), QNN_VER_PTR(*p_qnn_tensor)->memHandle);
        //handle = QNN_VER_PTR(*p_qnn_tensor)->memHandle;
        QNN_VER_PTR(*p_qnn_tensor)->memType = QNN_TENSORMEMTYPE_MEMHANDLE;
        QNN_VER_PTR(*p_qnn_tensor)->clientBuf = {.data=nullptr, .dataSize=0};
        //uint8_t * rpc_buf = ggmlqnn_create_rpc_buffer(instance, tensor, p_qnn_tensor, /* b_copydata= */ true);
        // QNN_VER_PTR(*p_qnn_tensor)->clientBuf = {.data=tensor->data, .dataSize=ggmlqnn_get_tensor_data_size(tensor)};
        // QNN_VER_PTR(*p_qnn_tensor)->clientBuf.dataSize = (uint32_t)ggml_nbytes(tensor);
        // if (rpc_buf == nullptr) {
        //     GGMLHEXAGON_LOG_ERROR("Failed to create rpc buffer for QNN tensor");
        //     return nullptr;
        // }
    } else {
        GGMLHEXAGON_LOG_DEBUG("not enable_qnn_rpc");
        QNN_VER_PTR(*p_qnn_tensor)->clientBuf = {data, data_size};
    }
    QNN_INTERFACE_VER_TYPE qnn_raw_interface = instance->get_qnn_raw_interface();
    //========debug start
    Qnn_Tensor_t * t = p_qnn_tensor;
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] name: %s", t->v1.name ? t->v1.name : "null");
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] type: %d", t->v1.type);
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] dataFormat: %d", t->v1.dataFormat);
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] dataType: %d", t->v1.dataType);
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] rank: %u", t->v1.rank);
    if (t->v1.rank == 0 || t->v1.dimensions == nullptr) {
        GGMLHEXAGON_LOG_WARN("[QNN_TENSOR DEBUG] Invalid tensor rank or dimensions!");
    }
    for (uint32_t i = 0; i < t->v1.rank; i++) {
        GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] dim[%u]: %u", i, t->v1.dimensions[i]);
    }
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] memType: %d", t->v1.memType);
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] clientBuf.data: %p", t->v1.clientBuf.data);
    GGMLHEXAGON_LOG_DEBUG("[QNN_TENSOR DEBUG] clientBuf.size: %u", t->v1.clientBuf.dataSize);
    //=======debug end
    CHECK_QNN_API(error, qnn_raw_interface.tensorCreateGraphTensor(graph_handle, p_qnn_tensor));
    GGMLHEXAGON_LOG_DEBUG("3_name : %s : mem_handle %p", (QNN_VER_PTR(*p_qnn_tensor)->name), QNN_VER_PTR(*p_qnn_tensor)->memHandle);
    GGMLHEXAGON_LOG_DEBUG("4_name : %s : mem_handle %p", (QNN_VER_PTR(*p_qnn_tensor)->name), QNN_VER_PTR(*p_qnn_tensor)->memHandle);
    return p_qnn_tensor;
}
...
void dump_tensor_debug(Qnn_Tensor_t* t, const char* label, int index) {
    if (t == nullptr) {
        GGMLHEXAGON_LOG_WARN("[%s %d] tensor is nullptr!", label, index);
        return;
    }

    GGMLHEXAGON_LOG_DEBUG("[%s %d] name        = %s", label, index, t->v1.name ? t->v1.name : "null");
    GGMLHEXAGON_LOG_DEBUG("[%s %d] type        = %d", label, index, t->v1.type);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] dataType    = %d", label, index, t->v1.dataType);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] dataFormat  = %d", label, index, t->v1.dataFormat);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] memType     = %d", label, index, t->v1.memType);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] rank        = %u", label, index, t->v1.rank);

    if (t->v1.dimensions == nullptr) {
        GGMLHEXAGON_LOG_WARN("[%s %d] dimensions is nullptr!", label, index);
    } else {
        for (uint32_t i = 0; i < t->v1.rank; ++i) {
            GGMLHEXAGON_LOG_DEBUG("[%s %d] dim[%u] = %u", label, index, i, t->v1.dimensions[i]);
        }
    }

    GGMLHEXAGON_LOG_DEBUG("[%s %d] clientBuf.data  = %p", label, index, t->v1.clientBuf.data);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] clientBuf.size  = %u", label, index, t->v1.clientBuf.dataSize);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] quantizeParams.encodingDefinition = %d", label, index, t->v1.quantizeParams.encodingDefinition);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] quantizeParams.scaleOffsetEncoding.scale = %f", label, index, t->v1.quantizeParams.scaleOffsetEncoding.scale);
    GGMLHEXAGON_LOG_DEBUG("[%s %d] quantizeParams.scaleOffsetEncoding.offset = %d", label, index, t->v1.quantizeParams.scaleOffsetEncoding.offset);
}
...
static void ggmlqnn_compute_mul_mat(ggml_backend_hexagon_context * ctx, ggml_tensor * op) {
    ...
    Qnn_Tensor_t * p_tensor1_transpose          = nullptr;
    ...
    if (ctx->qnn_singlenode_graph_map.find(graph_name) != ctx->qnn_singlenode_graph_map.end()) {
        ...
        GGMLHEXAGON_LOG_DEBUG("start1 : %lld microseconds", ggml_time_us() - start_1);
        int64_t start_2 = ggml_time_us();
        ...
        dump_tensor_debug(p_tensor1_transpose, "tensor1_after_transepose", 0);
        dump_tensor_debug(p_tensor2_transpose, "tensor2_after_transepose", 0);
        ...
        GGMLHEXAGON_LOG_DEBUG("3.....\n");
        //finalize QNN graph
        CHECK_QNN_API(error, qnn_raw_interface.graphFinalize(graph_handle, nullptr, nullptr));
        GGMLHEXAGON_LOG_DEBUG("4.....\n");
        //cache QNN graph
        ...
        ctx->qnn_singlenode_graph_map[graph_name] = graph_item;
        GGMLHEXAGON_LOG_DEBUG("start3 : %lld microseconds", ggml_time_us() - start_3);
    }
    GGMLHEXAGON_LOG_DEBUG("0.....\n");
    int64_t start_4 = ggml_time_us();
    if(instance->enable_qnn_rpc()) {
        uint8_t * rpc_buf_1 = ggmlqnn_create_rpc_buffer(instance, src0, p_tensor0, true);
        if (rpc_buf_1 == nullptr) {
        ...
        uint8_t * rpc_buf_3 = ggmlqnn_create_rpc_buffer(instance, dst, p_tensor2, true);
        if (rpc_buf_3 == nullptr) {
            GGMLHEXAGON_LOG_ERROR("Failed to create rpc buffer3 for QNN tensor");
            return ;
        }
    } else {
        if (src0_type != GGML_TYPE_F32) {
    ...
    GGMLHEXAGON_LOG_DEBUG("!!!!!!!!start4 : %lld microseconds", ggml_time_us() - start_4);
    //GGMLHEXAGON_LOG_DEBUG("rpcBuf src0: addr = %p", rpc_buf_1);
    //GGMLHEXAGON_LOG_DEBUG("rpcBuf src1: addr = %p", rpc_buf_2);
    //GGMLHEXAGON_LOG_DEBUG("rpcBuf dst : addr = %p", rpc_buf_3);
    GGMLHEXAGON_LOG_DEBUG("0-1.....\n");
    GGMLHEXAGON_LOG_DEBUG("4-1.....\n");
    // if (src0_type != GGML_TYPE_F32) {
    //     QNN_VER_PTR(*p_tensor0)->clientBuf = {wdata, static_cast<uint32_t>(desired_size)};
    // } else {
    //     QNN_VER_PTR(*p_tensor0)->clientBuf = {src0->data, ggmlqnn_get_tensor_data_size(src0)};
    // }
    // QNN_VER_PTR(*p_tensor0)->clientBuf = {(QNN_VER_PTR(*p_tensor0)->memHandle), ggmlqnn_get_tensor_data_size(src0)};
    // QNN_VER_PTR(*p_tensor1)->clientBuf = {(QNN_VER_PTR(*p_tensor1)->memHandle), ggmlqnn_get_tensor_data_size(src1)};
    // QNN_VER_PTR(*p_tensor2)->clientBuf = {(QNN_VER_PTR(*p_tensor2)->memHandle), ggmlqnn_get_tensor_data_size(dst)};
    // void * qnn_buffer_1 = instance->get_rpcmem_from_memhandle(QNN_VER_PTR(*p_tensor0)->memHandle);
    // void * qnn_buffer_2 = instance->get_rpcmem_from_memhandle(QNN_VER_PTR(*p_tensor1)->memHandle);
    // void * qnn_buffer_3 = instance->get_rpcmem_from_memhandle(QNN_VER_PTR(*p_tensor2)->memHandle);
    // QNN_VER_PTR(*p_tensor0)->clientBuf = {qnn_buffer_1, ggmlqnn_get_tensor_data_size(src0)};
    // QNN_VER_PTR(*p_tensor1)->clientBuf = {qnn_buffer_2, ggmlqnn_get_tensor_data_size(src1)};
    // QNN_VER_PTR(*p_tensor2)->clientBuf = {qnn_buffer_3, ggmlqnn_get_tensor_data_size(dst)};

    size_t size0 = ggmlqnn_get_tensor_data_size(src0);
    size_t size1 = ggmlqnn_get_tensor_data_size(src1);
    size_t size2 = ggmlqnn_get_tensor_data_size(dst);
    GGMLHEXAGON_LOG_DEBUG("clientBuf src0_not_32: addr = %p, size = %zu\n", wdata, static_cast<uint32_t>(desired_size));
    GGMLHEXAGON_LOG_DEBUG("clientBuf src0: addr = %p, size = %zu\n", src0->data, size0);
    GGMLHEXAGON_LOG_DEBUG("clientBuf src1: addr = %p, size = %zu\n", src1->data, size1);
    GGMLHEXAGON_LOG_DEBUG("clientBuf dst : addr = %p, size = %zu\n", dst->data,  size2);
    //GGMLHEXAGON_LOG_DEBUG("qnn_buffer_1: addr = %p, handle_address = %p\n", qnn_buffer_1, (QNN_VER_PTR(*p_tensor0)->memHandle));
    //GGMLHEXAGON_LOG_DEBUG("qnn_buffer_2: addr = %p, handle_address = %p\n", qnn_buffer_2, (QNN_VER_PTR(*p_tensor1)->memHandle));
    //GGMLHEXAGON_LOG_DEBUG("qnn_buffer_3: addr = %p, handle_address = %p\n", qnn_buffer_3,  (QNN_VER_PTR(*p_tensor2)->memHandle));
    ...
    GGMLHEXAGON_LOG_DEBUG("5.....\n");
    for (int i = 0; i < 2; ++i) {
        dump_tensor_debug(&tensor_inputs[i], "input", i);
    }
    for (int i = 0; i < 1; ++i) {
    ...
    int64_t start_5 = ggml_time_us();    
    CHECK_QNN_API(error, qnn_raw_interface.graphExecute(graph_handle,
                                                        tensor_inputs, 2,
                                                        tensor_outputs, 1,
                                                        nullptr, nullptr));
    GGMLHEXAGON_LOG_DEBUG("6.....\n");
    GGMLHEXAGON_LOG_DEBUG("start5 : %lld microseconds", ggml_time_us() - start_5);
    op_perf.info();
}
